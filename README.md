...
| 회계 | **선물은 일수를 새로 만들지 않고 사람 사이에서 옮기기만 합니다.** 발신은 `spent`(잔액용)와 `gsent`(누적용)를 함께 늘리고, 수령은 `gifted` 를 늘립니다.<br>· 잔액 = `total + gifted − spent`<br>· 누적 삭제한 일수(화면·리더보드) = `total + gifted − gsent`, 0 에서 자름 |
...
| 저장은 단조, 빼기는 표시할 때만 | `total`·`gifted`·`sent` 는 전부 늘기만 하므로 서버의 `greatest()` 병합이 그대로 성립합니다. 빼기는 `leaderboard()` 와 클라이언트의 `netTotal()` 에서만 합니다. |
