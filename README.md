...
| `crew_power` | 현재·이후 모든 병사의 1회 삭제량 추가 |
| `crew_speed` | 현재·이후 모든 병사의 자동 속도 `%p` 추가. 실제 간격은 기본 간격 ÷ 속도 |
| `leave` | 영수증을 만들고 정해진 종류·일수를 누적. 여러 번 구매 가능 |
| `share` | 위병소에서 성공한 공유 후 30분 동안 직접·자동 생산에 ×3 |

직접 클릭은 `(1 + 운동 추가치 합계) × 공유 배수`, 병사 1회는 `(1 + 음식 삭제량 추가치 합계) × 공유 배수`입니다. 속도는 음식의 `%p`를 합산합니다. 광고 보상·휴가 구매·선물은 생산 배수를 적용하지 않습니다.
...
`legacyItems`의 `multiplier`, `senior`, `offline`과 판매 종료한 자동 병사는 기존 보유자에게 계속 적용됩니다. 구 `roll`은 과거 당첨 결과가 잔액에 남으며 재판매·재지급하지 않습니다. 이 정의들은 새 상점에는 노출하지 않습니다. 기존 자동 효과는 별도의 legacy 지원으로 유지하며, 새 생활관에는 병사 6명만 표시합니다. `long-leave`는 계정별 결정 operation ID를 가진 `legacy-long-leave` 휴가 영수증으로 한 번만 이관합니다.
