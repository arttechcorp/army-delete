...
| 항목 | 내용 |
| --- | --- |
| 병합이 단순한 이유 | 네 값 모두 **단조 증가**합니다 — 일수는 클릭·광고 보상으로만 늘고, 사용액은 구매·선물 발신으로만 늘고, 받은 선물은 되돌려줄 수 없고, 아이템은 되팔 수 없습니다. 그래서 병합이 `max` / `max` / `max` / 합집합으로 끝납니다. 충돌 해소도, 타임스탬프 비교도, last-write-wins 도 필요 없고, 어느 기기가 먼저 도착하든 결과가 같습니다. |
...
| `p_gifted` 는 default 를 둠 | PostgREST 는 본문의 인자 **이름**으로 함수를 찾으므로, 아직 옛 코드를 들고 있는 브라우저가 4인자로 불러도 5인자 함수가 그대로 받습니다. default 없이 만들면 배포와 SQL 적용 사이에 동기화가 통째로 실패합니다. 대신 옛 4인자 판은 반드시 `drop` 해야 호출이 모호해지지 않습니다. |
...
보유 일수(`total − spent`)를 덜어 응원 메시지와 함께 링크로 보냅니다. 받은 사람은 링크를 열어 수령합니다. SQL 은 `supabase/gift.sql`, 검증은 `supabase/gift_test.sql`.
...
| 잔액 검증은 하지 않음 | 서버는 발신자가 정말 그 일수를 가졌는지 확인하지 않습니다. 잔액이 `localStorage` 에만 있어 확인할 방법이 없고, 지금도 콘솔 한 줄로 조작이 가능하므로 선물에만 방어선을 세우는 건 의미가 없습니다. |
...
| 회계 | 발신은 `spent` 를, **수령은 `gifted` 를** 늘립니다. 잔액은 `total + gifted − spent` 입니다. |
| 자기 자신에게 선물하기 | **막지 않습니다. 막을 필요가 없습니다.** 보낼 때 `spent` 가 늘고 받을 때 `gifted` 가 늘어 정확히 상쇄되므로, 자기 선물은 잔액도 순위도 바꾸지 않습니다. 예전처럼 수령이 `total` 을 올리면 잔액은 그대로인데 누적만 올라 **클릭 없이 리더보드를 무한히 올릴 수 있었습니다.** `claim_gift` 에서 발신자 여부를 검사하는 방식은 수령이 비로그인이라 시크릿 창 한 번에 뚫립니다 — 규칙으로 막는 대신 이득을 없앴습니다. |
| 받은 선물과 리더보드 | 리더보드는 `total_days` 만 집계합니다. 받은 일수는 내가 삭제한 일수가 아니므로 순위에 반영되지 않습니다. |
